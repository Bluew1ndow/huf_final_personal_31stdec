...
class HomeView(View):
    def get(self, request):
        ...
        try:
            ...
            #getting data for wrokAtGlance
            final_data.update({"workAtGlance": {
                "ourWorkFirstPara": key_value_table.objects.get(key='our_work_first_para').value,
                "ourWorkSecondPara": key_value_table.objects.get(key='our_work_second_para').value,
                "ourWorkThirdPara": key_value_table.objects.get(key='our_work_third_para').value,
            }})
            logger.info("our mission fetched")
